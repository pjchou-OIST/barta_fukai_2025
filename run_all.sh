...
STIM_RUN_CONFIG="${STIM_RUN_CONFIG}"
...
if [ $START_STEP -le 2 ] && [ $END_STEP -ge 2 ]; then
    ...
    JOB2_ID=$(sbatch \
        ...
        slurm_scripts/run_parallel_simulation.sh $SYSTEM_CONFIG $TRAIN_RUN_CONFIG $PATTERNS | awk '{print $4}')
...
if [ $START_STEP -le 3 ] && [ $END_STEP -ge 3 ]; then
    JOB3_ID=$(sbatch \
        ...
        slurm_scripts/run_parallel_simulation.sh $SYSTEM_CONFIG $SPONT_RUN_CONFIG $PATTERNS | awk '{print $4}')
...
if [ $START_STEP -le 5 ] && [ $END_STEP -ge 5 ]; then
    JOB5_ID=$(sbatch \
        ...
        slurm_scripts/run_parallel_simulation.sh $SYSTEM_CONFIG $COND_RUN_CONFIG $PATTERNS | awk '{print $4}')
...
if [ $START_STEP -le 7 ] && [ $END_STEP -ge 7 ]; then
    JOB7_ID=$(sbatch \
        ...
        slurm_scripts/run_parallel_simulation.sh $SYSTEM_CONFIG $PERT_RUN_CONFIG $PATTERNS | awk '{print $4}')
...
if [ $START_STEP -le 8 ] && [ $END_STEP -ge 8 ]; then
    ...
    DEP_STRING="--dependency=afterok:$JOB8_ID"
fi
...
if [ $START_STEP -le 9 ] && [ $END_STEP -ge 9 ]; then
    JOB9_ID=$(sbatch \
        $DEP_STRING \
        --job-name=step9_simuli100ms \
        --output="${LOG_DIR}/step9_simuli100ms_%j.out" \
        --error="${LOG_DIR}/step9_simuli100ms_%j.err" \
        slurm_scripts/run_parallel_simulation.sh $SYSTEM_CONFIG $STIM_RUN_CONFIG $PATTERNS | awk '{print $4}')
    echo "Step 9 (simuli100ms) submitted with Job ID: $JOB9_ID"
fi
