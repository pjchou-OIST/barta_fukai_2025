...
const size_t _numbasethr = 2000;
const size_t _numlastspike = 2000;
const double mV = 0.001;
const size_t _nummax_theta = 2000;
...
const size_t _numv = 2000;
    ...
    double* __restrict  _ptr_array_neurongroup_1_basethr = _array_neurongroup_1_basethr;
    double* __restrict  _ptr_array_neurongroup_1_lastspike = _array_neurongroup_1_lastspike;
    double* __restrict  _ptr_array_neurongroup_1_max_theta = _array_neurongroup_1_max_theta;
    ...
    for(size_t _idx=0; _idx<N; _idx++)
    {
        ...
        const double H1 = _ptr_array_neurongroup_1_H1[_idx];
        const double H2 = _ptr_array_neurongroup_1_H2[_idx];
        const double basethr = _ptr_array_neurongroup_1_basethr[_idx];
        const double max_theta = _ptr_array_neurongroup_1_max_theta[_idx];
        const char not_refractory = _ptr_array_neurongroup_1_not_refractory[_idx];
        const double v = _ptr_array_neurongroup_1_v[_idx];
        const double theta = (basethr + H1) + H2;
        char _cond;
        if(!not_refractory)
            _cond = (v > _clip(theta, _lio_1, max_theta)) && false;
        else 
            _cond = (v > _clip(theta, _lio_1, max_theta)) && true;
