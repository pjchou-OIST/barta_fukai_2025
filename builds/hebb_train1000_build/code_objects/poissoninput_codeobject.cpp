...
namespace {
    ...
    {
        long X = 0;
        double px = 0.818714376443118;
        double U = _rand(vectorisation_idx);
        while (U > px)
        {
            X++;
            if (X > 11.154268574396008)
            {
                X = 0;
                px = 0.818714376443118;
                U = _rand(vectorisation_idx);
            } else
            {
                U -= px;
                px = ((1000-X+1) * 0.000200000000000 * px)/(X*0.999800000000000);
            }
        }
        return X;
    }
...
const double psiemens = 1e-12;
    ...
    const double _lio_1 = 300.0 * psiemens;
    ...
    for(int _idx=0; _idx<_N; _idx++)
    {
        ...
        ge += _lio_1 * poissoninput_binomial(_vectorisation_idx);
